...
export class UpdateResourceService {
  ...
  async perform(realm: string, accessToken: string, resource: Resource): Promise<boolean> {
    try {
      if (!resource.name || !resource.displayName) throw Error('Name or displayName is missing')

      const response = await this.resourceClient.update(realm, accessToken, resource)
